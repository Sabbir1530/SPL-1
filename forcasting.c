...
float predictRain(float humidity, float wind_speed, float pressure, double *coefficients)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        dotProduct += coefficients[i] * features[i];
    }
...
// Trin the model (manually Initialize)
/*void trainModel(WeatherData *trainSet, int trainCount, double *coefficients)
{
    coefficients[0] = 0.5;  // humidity
    coefficients[1] = 0.3;  // wind speed
    coefficients[2] = -0.1; // pressure
}*/
